    ...
    <header className="w-full bg-white dark:bg-gray-900 shadow-md sticky top-0 z-50 transition-colors duration-300">
      <div className="container mx-auto px-4 md:px-8 h-20 flex justify-between items-center">
        ...
        <nav className="hidden md:flex gap-x-8 items-center">
          {navLinks.map((link) => {
            const isActive = pathname && pathname.replace(/\/$/, "") === link.href.replace(/\/$/, "");
              ...
              >
                ...
                <span
                  className={`inline-block h-5 w-0.5 mr-3 rounded transition-transform duration-300 origin-top
                    ${isActive ? "scale-y-100 bg-red-600 dark:bg-red-400" : "scale-y-100 bg-blue-900/20 dark:bg-gray-100/20"}
                  `}
                >
                </span>
